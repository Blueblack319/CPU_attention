...
template <typename T>
void key_gemv_eval(const int K, const int Dh, const int num_head,
                   const int batch_size, const int iteration,
  ...
  for (int ii = 0; ii < iteration; ++ii) {
    for (int i = 0; i < num_head * batch_size * K; ++i) {
      logits[ii][i] = 0.f;
...
void key_gemv_eval<std::uint16_t>(
    const int K, const int Dh, const int num_head, const int batch_size,
    const int iteration, const int keys_head_offset,
  ...
  for (int t = 0; t < num_threads; ++t) {
    ...
    threads.emplace_back(
        key_gemv_threaded_half, keys, queries, logits, num_head, batch_size, K,
